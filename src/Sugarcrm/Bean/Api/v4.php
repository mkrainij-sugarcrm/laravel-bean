 ...
 *
 * @TODO - in case of any changes, make sure to update app/controllers/ItPortal/NewHireController.php
 *
 *
 * @package     SugarCRM
 * @category    Libraries
...
class v4 {
     ...
     */
    function connect($rest_url=null,$username=null,$password=null,$md5_password=false)
    {
        if (!is_null($rest_url))
        {
    ...
    private function rest_request($call_name, $call_arguments) {
        ...
        curl_setopt($ch, CURLOPT_COOKIE, 'iapps=true');
        $output = curl_exec($ch);
    ...
    public function get_entry($module, $id, $fields = null)
    {
        ...
        if (!is_array($result)) {
            $this->error['name']        = "Empty response or parse error";
            $this->error['number']      = -1;
            $this->error['description'] = "We are having technical difficulties. We apologize for the inconvenience.";
     ...
     */
    public function retrieve($module, $id, $options = null)
    {
        $results = $this->get_entry($module, $id, $options);
        $records = array();
        if ($results) {
            $records = $this->parseNameValueList($results['entry_list'][0]['name_value_list']);
        }
        return $records;
    }
     ...
     *       'name' field in the first record would be accessed in $result[0]['name].
     */
    public function get($module,$fields,$options=null, $related=null) {

        $fieldsByModule = array($module => $fields);
        if($related != null){
            $fieldsByModule = array_merge($fieldsByModule, $related);
        }
        $results = $this->get_with_related($module,$fieldsByModule,$options);
        $records = array();
        ...
        if ($results) {
            ...
            if(isset($results['relationship_list'])){
                foreach($results['relationship_list'] as $i => $entry){
                    $record = array();
                    foreach($entry['link_list'] as $relatedModule){
                        $linkName = $relatedModule['name'];
                        foreach($relatedModule['records'][0]['link_value'] as $field) {
                            $record[$linkName][$field['name']] = $field['value'];
                        }
                    }
                    $records[$i]["relationships"] = $record;
                }
            }
        }
        ...
        $final->total      = (int)$results["total_count"];
        return $final;
    }
    ...
    /**
     * Function: Allows convert SugarCRM's output into array
     *
     * @param $list
     *
     * @return array of results
     */
    public function parseNameValueList($list)
    {
        $record = array();
